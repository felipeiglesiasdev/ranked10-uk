...
<p class="flex items-center gap-2 text-sm text-slate-500">{{-- LINHA DE METADADOS EM CINZA --}}
    ...
    <span>By <span class="font-medium text-slate-700">{{ $author }}</span> &middot; Updated <time datetime="{{ $date->toDateString() }}">{{ $date->format('j F Y') }}</time></span>{{-- AUTOR E DATA NO FORMATO BRITANICO --}}
</p>
