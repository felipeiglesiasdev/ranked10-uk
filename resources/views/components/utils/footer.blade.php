@props(['navCategories' => collect(), 'popularArticles' => collect()]){{-- PROPS: CATEGORIAS E GUIAS RECENTES, AMBOS JA PRONTOS --}}

{{-- OS "GUIAS POPULARES" CHEGAM PRONTOS DO AppServiceProvider.
     ANTES ELES ERAM DERIVADOS AQUI, COM flatMap SOBRE OS ARTIGOS QUE VINHAM DENTRO DAS
     CATEGORIAS. ISSO SO FUNCIONAVA PORQUE O MEGA MENU JA CARREGAVA OS 76 ARTIGOS E OS 760
     PRODUTOS EM TODA REQUISICAO. COM O MENU AGORA ASSINCRONO, ESSES DADOS NAO EXISTEM MAIS
     NO NAVEGADOR DAS CATEGORIAS, E O RODAPE PASSOU A RECEBER SUA PROPRIA CONSULTA ENXUTA —
     6 ARTIGOS, COM A CATEGORIA JA CARREGADA JUNTO. --}}
...
<footer class="mt-16 bg-ink text-slate-300" role="contentinfo">{{-- RODAPE ESCURO (PRETO DA MARCA) --}}
    <div class="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 py-12">
        <div class="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">{{-- GRID QUE COLAPSA: 1 -> 2 -> 4 COLUNAS --}}
            ...
            <div class="sm:col-span-2 lg:col-span-1">{{-- OCUPA LARGURA TOTAL ATE O LG --}}
                <a href="{{ route('home') }}" class="inline-flex items-center gap-2" aria-label="ranked10 — homepage">{{-- LOGO DO RODAPE --}}
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16" class="text-brand-on-dark" aria-hidden="true"><path d="M2.5.5A.5.5 0 0 1 3 0h10a.5.5 0 0 1 .5.5q0 .807-.034 1.536a3 3 0 1 1-1.133 5.89c-.79 1.865-1.878 2.777-2.833 3.011v2.173l1.425.356c.194.048.377.135.537.255L13.3 15.1a.5.5 0 0 1-.3.9H3a.5.5 0 0 1-.3-.9l1.838-1.379c.16-.12.343-.207.537-.255L6.5 13.11v-2.173c-.955-.234-2.043-1.146-2.833-3.012a3 3 0 1 1-1.132-5.89A33 33 0 0 1 2.5.5m.099 2.54a2 2 0 0 0 .72 3.935c-.333-1.05-.588-2.346-.72-3.935m10.083 3.935a2 2 0 0 0 .72-3.935c-.133 1.59-.388 2.885-.72 3.935"/></svg>
                    <span class="text-lg font-extrabold text-white">ranked<span class="text-brand-on-dark">10</span></span>{{-- NOME DO SITE — VERMELHO CLARO PORQUE O VERMELHO DA MARCA REPROVA NO CONTRASTE SOBRE O PRETO --}}
                </a>
                ...
                <p class="mt-3 text-xs leading-relaxed text-slate-400">{{-- DISCLOSURE CURTO DE AFILIADO --}}
            ...
            <nav class="text-sm" aria-label="Footer categories">{{-- LINKS DAS CATEGORIAS (GRAFO DE LINKS) --}}
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Categories</p>{{-- TITULO DA COLUNA --}}
                <ul class="mt-3 space-y-2">{{-- LISTA DE CATEGORIAS --}}
                    ...
                    @empty
                        <li class="text-slate-400">Coming soon</li>{{-- MENSAGEM SE VAZIO --}}
                    @endforelse
                </ul>
            </nav>
            ...
            <nav class="text-sm" aria-label="Popular guides">{{-- LINKS DE ARTIGOS PARA FECHAR O GRAFO DE LINKS --}}
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Popular top 10 guides</p>{{-- TITULO DA COLUNA --}}
                <ul class="mt-3 space-y-2">{{-- LISTA DE ARTIGOS --}}
                    ...
                    @empty
                        <li class="text-slate-400">Guides coming soon</li>{{-- MENSAGEM SE VAZIO --}}
                    @endforelse
                </ul>
            </nav>
            ...
            <nav class="text-sm" aria-label="Legal">{{-- LINKS LEGAIS --}}
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Company</p>{{-- TITULO DA COLUNA --}}
                <ul class="mt-3 space-y-2.5">{{-- LISTA DE LINKS LEGAIS --}}
                    <li>
                        <a href="{{ route('about') }}" class="inline-flex items-center gap-2 text-slate-400 hover:text-white">{{-- PAGINA PROPRIA DE SOBRE (ANTES ERA SO UMA ANCORA DENTRO DA POLITICA DE PRIVACIDADE) --}}
                            {{-- ICONE INFO (BOOTSTRAP ICONS: INFO-CIRCLE) EM SVG INLINE --}}
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" class="text-slate-400" aria-hidden="true"><path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/><path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0"/></svg>
                            About
                    ...
                    <li>
                        <a href="{{ route('privacy') }}#affiliate" class="inline-flex items-center gap-2 text-slate-400 hover:text-white">{{-- DISCLOSURE DE AFILIADO COMO ANCORA --}}
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" class="text-slate-400" aria-hidden="true"><path d="M6 4.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0M5 4a.5.5 0 1 0 0-1 .5.5 0 0 0 0 1"/><path d="M2 1h4.586a1 1 0 0 1 .707.293l7 7a1 1 0 0 1 0 1.414l-4.586 4.586a1 1 0 0 1-1.414 0l-7-7A1 1 0 0 1 1 6.586V2a1 1 0 0 1 1-1m0 5.586 7 7L13.586 9l-7-7H2z"/></svg>
        ...
        <div class="mt-10 border-t border-slate-800 pt-6 space-y-2 text-xs leading-relaxed text-slate-400">{{-- RODAPE INFERIOR COM AVISOS LEGAIS --}}
