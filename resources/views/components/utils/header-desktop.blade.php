        ...
        <div class="flex items-center justify-between gap-4 h-16">{{-- LINHA SUPERIOR COM LOGO E BUSCA --}}
            ...
            <form action="{{ route('search') }}" method="GET" class="flex items-center flex-1 max-w-xs" role="search">{{-- FORMULARIO DE BUSCA DO DESKTOP --}}
                ...
                <div class="relative w-full">
                    ...
                    <input id="desktop-search" type="search" name="q" value="{{ request('q') }}" placeholder="Search top 10 lists..." class="w-full rounded-full border border-slate-200 bg-slate-50 py-2 pl-9 pr-4 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-200">{{-- CAMPO DE BUSCA DO HEADER --}}
                </div>
            </form>
        </div>
