...
@section('content'){{-- INICIO DO CONTEUDO DA HOME --}}
    ...
    <section class="bg-slate-900 text-white">{{-- HERO ESCURO DE ABERTURA --}}
        <div class="max-w-6xl mx-auto px-4 py-16 md:py-24 text-center">
            <h1 class="text-3xl md:text-5xl font-extrabold tracking-tight">Find the best products in the UK,<br class="hidden md:block"> <span class="text-amber-400">ranked for you.</span></h1>{{-- TITULO PRINCIPAL DO HERO --}}
            ...
            <div class="mt-8 flex flex-wrap justify-center gap-3">{{-- BOTOES PARA AS CATEGORIAS --}}
                @foreach ($categories as $category){{-- PERCORRE AS CATEGORIAS PARA CRIAR OS ATALHOS --}}
                    <a href="{{ route('category', $category) }}" class="rounded-full border border-slate-700 bg-slate-800 px-5 py-2 text-sm font-medium hover:border-amber-400 hover:text-amber-400">{{ $category->name }}</a>{{-- ATALHO DA CATEGORIA --}}
                @endforeach
            </div>
        </div>
    </section>
    ...
    <section class="max-w-6xl mx-auto px-4 py-12">{{-- SECAO DE ARTIGOS EM DESTAQUE --}}
        ...
        <div class="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{{-- GRID RESPONSIVO DE CARDS --}}
            @forelse ($featured as $article){{-- PERCORRE OS ARTIGOS EM DESTAQUE --}}
                <a href="{{ route('article', [$article->category, $article]) }}" class="group flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md hover:border-amber-300">{{-- CARD CLICAVEL DO ARTIGO --}}
                    <span class="inline-flex w-fit items-center rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">{{ $article->category->name }}</span>{{-- BADGE DA CATEGORIA --}}
                    <h3 class="mt-3 text-lg font-bold text-slate-900 group-hover:text-amber-600">{{ $article->title }}</h3>{{-- TITULO DO ARTIGO --}}
